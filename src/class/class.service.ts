...
import { ClassResponseDto, ClassResponseWithRelationsDto } from "./class.response";
...
export class ClassService {
  ...
  async validateTeacherExist(formTeacherId: number) {
    ...
    if (!teacher) {
      throw new NotFoundException(`Không tìm thấy giáo viên với ID ${formTeacherId}`);
    }
    return teacher;
  }
   ...
   * Tạo lớp học
   */
  async create(data: CreateClassDto, tx?: Prisma.TransactionClient): Promise<ClassResponseDto> {
   ...
   */
  async findAll(query: SearchClassDto, user?: any): Promise<ClassResponseDto[]> {
